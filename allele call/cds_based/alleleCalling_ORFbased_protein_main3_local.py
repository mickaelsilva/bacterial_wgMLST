...
def main():
	...
	for genome in listOfGenomes:
		pool.apply_async(call_proc, args=(["./runProdigal.py",str(genome),basepath,prodigalPath],))
	...
	for genomeFile in listOfGenomes:
		...
		pool.apply_async(call_proc, args=(["./Create_Genome_Blastdb.py",filepath,os.path.join(basepath,str(os.path.basename(genomeFile)) ),str(os.path.basename(genomeFile))],))
	...
	for argList in argumentsList:
		...
		pool.apply_async(call_proc, args=(["./callAlleles_protein3.py",str(argList),basepath],))
