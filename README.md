...
2. Run CreateSchema.py over the concatenated single fasta file, save all the gene .fasta files inside a new "genes" folder
2.1 Create a folder "short" inside the "genes" folder, with a duplication of the .fasta genes files. Run ```for file in *; do mv "$file" "`basename $file .fasta`_short.fasta"; done``` inside the folder
...
Performing a **local** or **SLURM cluster** allele call (uses the maximum number of local cores minus 2, one per gene):
...
Performing a **SUN cluster** allele call (uses all available cores, one per gene):
